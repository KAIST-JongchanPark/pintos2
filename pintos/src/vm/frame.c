...
list_pop_max (struct list *list)
{
  ...
  while(!is_tail(curr_elem)) 
  {
      if(((list_entry (max_elem, struct frame_table_entry, elem) -> counter) < (list_entry (curr_elem, struct frame_table_entry, elem) -> counter)) && !(spt_get_page((list_entry (curr_elem, struct frame_table_entry, elem) -> counter) -> upage) -> swapped))
	  {
        max_elem = curr_elem;
	  }
	  curr_elem = list_next(curr_elem);
